...
export default function HomePage() {
  ...
  }, [doctors]); // re-run when doctors load

  const specialities = [
    { name: "Urology",      img: Urology,      icon: FaRunning,      filter: "Urology"       },
    { name: "Orthopedic",   img: Orthopedic,   icon: FaBone,         filter: "Orthopedics"   },
    { name: "Cardiologist", img: Cardiologist, icon: FaHeartbeat,    filter: "Cardiology"    },
    { name: "Dentist",      img: Dentist,      icon: FaStethoscope,  filter: "General Practice" },
    { name: "Neurology",    img: Neurology,    icon: FaBrain,        filter: "Neurology"     },
    { name: "Pediatric",    img: Pediatrist,   icon: FaBaby,         filter: "Pediatrics"    },
  ];

  const handleSpecialityClick = (filter) => {
    navigate(`/doctors?specialisation=${filter}`);
  };
    ...
    <div className="flex flex-col min-h-screen bg-[#F6FAFF]">
          ...
          <div className="relative max-w-lg mt-20 ml-12">
            ...
            <div className="mt-6 flex flex-col md:flex-row gap-2">
              <input
                ...
                onKeyDown={(e) => {
                  if (e.key === "Enter") navigate(`/doctors?search=${e.target.value}`);
                }}
              />
              <button
                className="bg-blue-600 text-white px-6 py-1 rounded-full hover:bg-blue-700"
                onClick={(e) => {
                  const input = e.target.previousSibling;
                  navigate(`/doctors?search=${input.value}`);
                }}
              >
                Search
              </button>
      ...
      <section className="px-10 py-16 bg-white">
        <div className="max-w-[calc(5*180px+4*24px)] mx-auto mb-12 flex justify-between items-center">
          <div>
            <h3 className="text-3xl font-bold text-[#012047] mb-6">Find by Speciality</h3>
        ...
        {!showAllSpecialities && (
          <div className="flex justify-center">
            <div
              ...
              style={{ maxWidth: "calc(5 * 180px + 4 * 24px)", scrollbarWidth: "none", msOverflowStyle: "none" }}
            >
              <style>{`div::-webkit-scrollbar { display: none; }`}</style>
              {specialities.map((item, i) => (
                <div
                  key={i}
                  onClick={() => handleSpecialityClick(item.filter)}
                ...
                >
                  <div className="absolute inset-0 bg-blue-600 opacity-0 group-hover:opacity-70 transition-opacity duration-300" />
        ...
        {showAllSpecialities && (
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-6 px-4 md:px-10 mt-8">
            {specialities.map((item, i) => (
              ...
              >
                <div className="absolute inset-0 bg-blue-600 opacity-0 hover:opacity-70 transition-opacity duration-300" />
      ...
      <div className="w-full flex justify-center py-10">
        <img src={Banner2} alt="Banner" className="w-full max-w-6xl object-contain" />
      </div>
      ...
      <section className="px-10 py-16 bg-white">
        <div className="max-w-6xl mx-auto grid md:grid-cols-2 gap-10 items-start">
          <div className="flex justify-center md:sticky md:top-10 h-fit">
            <img src={FAQ} alt="FAQ" className="w-full max-w-md object-contain" />
          </div>
          <div>
            ...
            {[
              { q: "How do I book an appointment with a doctor?", a: "Simply visit our website and log in or create an account. Search for a doctor based on specialization, location, or availability and confirm your booking." },
              { q: "Can I request a specific doctor when booking?",    a: "Yes, you can choose your preferred doctor while making the appointment." },
              { q: "What should I do if I need to cancel or reschedule?", a: "You can cancel or reschedule your appointment from your dashboard." },
              { q: "What if I'm running late for my appointment?",     a: "Please inform the clinic as soon as possible. Your slot may be adjusted." },
              { q: "Can I book for family members?",                   a: "Yes, you can book appointments for dependents using your account." },
            ].map((item, i) => (
              <details key={i} className="mb-3 bg-blue-50 rounded-lg p-4 cursor-pointer group">
